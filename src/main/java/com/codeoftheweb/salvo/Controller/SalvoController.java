...
public class SalvoController {
    ...
    public ResponseEntity<Map<String, Object>> StoreSalvoes(@PathVariable long gamePlayerId, @RequestBody Salvo salvos, Authentication authentication) {
        ...
        if (gamePlayer.getSalvoes().size() != opponent.get().getSalvoes().size() && gamePlayer.getSalvoes().size() > opponent.get().getSalvoes().size()) {
            return new ResponseEntity<>(makeMap("error", "salvoes already submitted for turn" + gamePlayer.getSalvoes().size()), HttpStatus.FORBIDDEN);
        }
        if (salvos.getSalvoLocations().size() == 0) {
        ...
        if (salvos.getSalvoLocations().size() > 5) {
            return new ResponseEntity<>(makeMap("error", "You have to submit a salvo and a max of 5"), HttpStatus.FORBIDDEN);
        }
